...
const HomePage = () => {
    ...
    const [environmentDescription, setEnvironmentDescription] = useState("Click on a land plot to get your farm judged by EnvirAI.")
    const [locationScore, setLocationScore] = useState ("?");
    const [locationDescription, setLocationDescription] = useState("Click on a land plot to get your farm judged by EnvirAI.")
    ...
    const flipModal = () => {
        ...
        if(currentObjects.length > 1) {
            setModal(!visible);
                ...
                notification.open({
                description: "Please place some plots on the map first! EnvirAI can't analyze empty fields!",
    ...
    useEffect(async () => {
        const response = await fetch(
            "http://localhost:3001/data/earth",
            {
                method: "GET",
                headers: {
